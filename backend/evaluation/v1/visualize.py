...
class HaystackRAGEvaluator:
    ...
    def __init__(
        ...
        if self.use_sas:
            try:
                self.sas_eval = SASEvaluator()
            except Exception as e:
                print(f"SAS evaluator initialization failed: {e}")
                self.use_sas = False
        ...
        self.use_llm_evaluator = use_llm_evaluator and LLM_EVAL_AVAILABLE
    ...
    def _calculate_similarity(self, reference: str, candidate: str) -> float:
        ...
        try:
            if self.use_sas:
                # Use SAS evaluator if available - Haystack 0.42 format
                result = self.sas_eval.run(
                    ground_truth_answers=[reference], predicted_answers=[candidate]
                )
                return float(result.get("score", 0.0))
            elif self.use_llm_evaluator:
    ...
    async def evaluate_single(
        ...
        try:
            ...
            return HaystackEvaluationResult(
                ...
                sas_score=similarity_score if self.use_sas else None,
    ...
    def _results_to_dataframe(
        ...
        for result in results:
            ...
            if result.sas_score is not None:
                row["sas_score"] = result.sas_score
    ...
    def calculate_metrics(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "sas_score" in df.columns:
            metrics["avg_sas_score"] = df["sas_score"].mean()
