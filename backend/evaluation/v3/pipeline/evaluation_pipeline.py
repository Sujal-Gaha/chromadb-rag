...
class EvaluationPipeline:
    ...
    def _extract_original_from_temp_filename(
        ...
        try:
            ...
            if title_match:
                ...
                filename = (
                    re.sub(r"[^\w\-]", "", title.lower().replace(" ", "-")) + ".txt"
                )
                return filename
            lines = content.split("\n")
            for line in lines[:20]:
                if "title:" in line.lower():
                    ...
                    if title:
                        filename = (
                            re.sub(r"[^\w\-]", "", title.replace(" ", "-")) + ".txt"
                        )
                        return filename
            content_hash = abs(hash(content[:200]))
            return f"untitled-document-{content_hash}.txt"
            ...
            try:
                ...
                for evaluator_name, evaluator in self.evaluators.items():
                    try:
                        ...
                        log.error(
                            f"Evaluator {evaluator_name} failed: {e}", exc_info=True
                        )
                ...
                retries += 1
                log.warning(
                    f"Query failed (attempt {retries}/{self.max_retries}): {e}. Retrying in {self.retry_delay}s..."
                )
                ...
                if retries >= self.max_retries:
                    log.error(
                        f"Max retries exceeded for question: {question[:50]}... - {e}",
                        exc_info=True,
                    )
    ...
    def _aggregate_metrics(self, questions: list[QuestionResult]) -> dict[str, Any]:
        ...
        for question in questions:
            ...
            for result in question.evaluation_results:
                ...
                # Per-difficulty
                if metric_key not in difficulty_metrics[diff]:
                    difficulty_metrics[diff][metric_key] = []
                difficulty_metrics[diff][metric_key].append(result.value)
        ...
        aggregated["per_difficulty"] = {}
        for diff, metrics in difficulty_metrics.items():
            aggregated["per_difficulty"][diff] = {}
            for metric_name, values in metrics.items():
                if values:
                    aggregated["per_difficulty"][diff][f"{metric_name}_mean"] = sum(
                        values
                    ) / len(values)
    ...
    def get_summary_report(self, batch_results: BatchResult) -> dict[str, Any]:
        ...
        for key, value in batch_results.aggregated_metrics.items():
            ...
            if "_mean" in key and ("recall" in key or "precision" in key):
                summary["top_metrics"][key] = value

        if "per_difficulty" in batch_results.aggregated_metrics:
            summary["per_difficulty_metrics"] = batch_results.aggregated_metrics[
                "per_difficulty"
            ]

        return summary

    def _export_results(self, batch_result: BatchResult, batch_id: str):
        output_dir = "evaluation_results"
        os.makedirs(output_dir, exist_ok=True)
        df = self.results_to_dataframe(batch_result)
        csv_path = os.path.join(output_dir, f"results_{batch_id}.csv")
        df.to_csv(csv_path, index=False)
        log.info(f"Exported detailed results to: {csv_path}")

        summary = self.get_summary_report(batch_result)
        json_path = os.path.join(output_dir, f"summary_{batch_id}.json")
        import json

        with open(json_path, "w") as f:
            json.dump(summary, f, indent=2)
        log.info(f"Exported summary to: {json_path}")
